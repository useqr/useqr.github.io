...
function chooseBestModeData(data) {
   ...
   try { return new QRCode.QRKanji(data); } catch (error) { return new QRCode.QRByte(data); }
	...
	var kosong = urlasli.replace(/sq?=.+/,"sq?="); if (urlasli == kosong) {window.location="/404"}
	var takde = kosong.replace('sq?=',''); if (urlasli == takde) {window.location="/404"}
	var akh = urlasli.slice(-1); if (akh == "-") {window.location="/404"}
	var dataUrl = urlasli.replace(/.+sq?=/,"").split("-");
	console.log(dataUrl);
	var posts = '';   
